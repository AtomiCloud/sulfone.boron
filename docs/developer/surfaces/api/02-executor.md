...
| Field             | Type                   | Required    | Description                                                   |
| ----------------- | ---------------------- | ----------- | ------------------------------------------------------------- |
| `sessionId`       | `string`               | Yes         | Unique session identifier (generated by Iridium)              |
| `localTemplateId` | `string`               | Yes         | Synthetic local template ID (e.g., `local-{uuid}`)            |
| `source`          | `string`               | No          | Blob source: `"image"` (default) or `"path"` (for --dev mode) |
| `imageRef`        | `DockerImageReference` | Conditional | Required when `source="image"`. Local Docker image reference  |
