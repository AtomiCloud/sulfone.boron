...
Currently, MERGER in Boron uses a simple Last-Write-Wins (LWW) strategy when multiple processors output same file. This spec defines changes to enable MERGER to call resolver containers to intelligently merge conflicting files.

## Context

### Current Behavior

When multiple processors output same file path (e.g., `package.json`), MERGER uses Last-Write-Wins: last processor's output overwrites all previous versions. This can cause data loss when processors contribute different sections to same file.
...
4. Apply resolved result to merge output
5. Continue with plugin execution on resolved output
...
**Implication:** If multiple resolvers could match same conflicted file, this indicates a template configuration mistake → ERROR (not a fallback scenario like Iridium's cross-template LWW).
...
**Example:** If `package.json` appears in 3 processors, send 1 request with 3 file versions. If `config.json` also conflicts, send a second request with all its versions.

### AC4: Resolver Request Format

For each conflicted file, resolver receives a request containing ALL versions of that file.
...
The resolver returns resolved content for conflicted file.

**Expected:** Resolved content for input file path.

The MERGER must apply resolved result to merge output directory.

### AC6: Handle Non-Matched Conflicts (LWW)

Conflicted files that match ZERO resolvers use LWW.

- For each conflicted file with no resolver match, use last processor's version
...
- This library provides globstar (`**`) support compatible with Helium/Iridium
- Supported patterns: `*`, `**` (recursive), `?`, `[...]`, `{a,b}` (brace expansion)
...
- Examples:
  ...
  - `src/**/*.go` - matches `.go` files in `src/**/` directory (recursive `**`)
...
- Resolver containers are on `cyanprint` network
