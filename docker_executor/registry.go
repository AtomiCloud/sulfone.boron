...
func (rc RegistryClient) convertProcessor(cp CyanProcessorReq, processors []ProcessorRes) (CyanProcessor, error) {
	...
	if version == nil {
		return CyanProcessor{}, fmt.Errorf("processor %s must have a pinned version (expected format: username/name:version)", n)
	} else {
		v := *version
...
func (rc RegistryClient) convertPlugin(cp CyanPluginReq, plugins []PluginRes) (CyanPlugin, error) {
	...
	if version == nil {
		return CyanPlugin{}, fmt.Errorf("plugin %s must have a pinned version (expected format: username/name:version)", n)
	} else {
		v := *version
